...
export class SnsService implements ISnsService {
  ...
  constructor() {
    ...
    if (!this.topicArn) {
      ...
      console.error(
        "ERROR DE CONFIGURACIÓN: APPOINTMENT_TOPIC_ARN no está definido."
      );
    }
  ...
  async publishAppointmentRequest(
    ...
    console.log(
      `[SNS] Publicando agendamiento ${appointmentId} para ${data.countryISO}.`
    );
    ...
    try {
      ...
      console.error(
        `[SNS] ERROR al publicar la solicitud ${appointmentId}:`,
        error
      );
      throw new Error(
        `Fallo en el servicio de SNS para cita ${appointmentId}.`
      );
    }
  }
}
